...
MongoProvider.prototype.init =function(connStr, opt, callback) {
    ...
    if (connStr.indexOf('mongodb://')!=0) connStr='mongodb://'+connStr;
    var self = this;
                ...
                for (var c in upd) {
                    for (var n in upd[c]) {
                        op.push(
                            (function (col, upd) {
                                return function (_cb) {
                                    self.db.command({ update: col, updates: upd, ordered: true, writeConcern: { w: 0 } }, _defaultCmdOpt, _cb);
                                }
                            })(c, upd[c][n])
                        )
                    }
                ...
                for (var i in ins) {
                    for (var n in ins[i]) {
                        op.push(
                            (function (col, ins) {
                                return function (_cb) {
                                    self.db.command({ insert: col, documents: ins, ordered: true, writeConcern: { w: 0 } }, _defaultCmdOpt, _cb);
                                }
                            })(i, ins[i][n])
                        )
                    }
